...
##
## Compare Taylor expansions @var{X} and @var{Y} for equality.
##
## True if @var{X} and @var{Y} are of the same order and have the
...
function result = eq (x, y)
  ...
  result = all (x.coefs == y.coefs, 1);
  result = reshape (result, [size(result)(2:end), 1]);

endfunction

%!assert (eq (taylor (), taylor ()))
%!assert (!eq (taylor (), taylor (infsupdec (1), 1)))
%!assert (eq (taylor (infsupdec (2), 3), taylor (infsupdec (2), 3)))

%!assert (eq (taylor (infsupdec (magic (3))), taylor (infsupdec (magic (3)))))
%!test
%! x = taylor (infsupdec (magic (3)), 3);
%! y = x;
%! y(1) = taylor (infsupdec (10), 3);
%! assert (eq (x, y), logical ([0 1 1; 1 1 1; 1 1 1]));
